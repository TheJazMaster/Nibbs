...
internal sealed class CustomTTGlossary : TTGlossary
{
	...
	private static bool TTGlossary_MakeNameDescPair_Prefix(string nameColor, ref string __result)
	{
		...
		if (glossary.key == ("status." + Instance.BacktrackLeftStatus.Status.Key())) {
			var left = glossary.GetLeftRightTooltipKeys("status.evade");
			if (left.HasValue)
				glossary.vals = [left.Value.left];
		}
		else if (glossary.key == ("status." + Instance.BacktrackRightStatus.Status.Key())) {
			var right = glossary.GetLeftRightTooltipKeys("status.evade");
			if (right.HasValue)
				glossary.vals = [right.Value.right];
		}
